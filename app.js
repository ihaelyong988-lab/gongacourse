...
/* 저장에는 '방문자가 만든 것'만 담는다. 코스 본문(제목·동선·맛집 261KB)은 defaultCourses에 이미 있는데,
   투표 한 번·후기 한 줄마다 그 261KB를 후기 사진과 함께 통째로 다시 써 왔다 — 용량이 찬 기기에서는
   그 한 번의 쓰기가 통째로 거부돼 방금 쓴 후기가 그대로 사라진다.
   여기서는 시드와 달라진 코스의 기록(후기·사진·투표)만 남기고, 부팅부가 defaultCourses 위에
   id로 얹어 되살린다(mergeVisitorRecords).
   ⚠ 부팅부의 "저장본 길이 ≠ defaultCourses 길이 → 재적재 + 병합 복원" 판정과 한 덩어리다.
      그 길이 검사를 없애면 본문 없는 코스 배열이 그대로 화면에 올라간다. */
let defaultRecordJson = null;
function visitorRecordOf(c) {
  return { id: c.id, votesUp: c.votesUp, votesDown: c.votesDown, comments: c.comments || [], photos: c.photos || [] };
}
function saveToLocalStorage() {
  if (!defaultRecordJson) {
    defaultRecordJson = new Map();
    defaultCourses.forEach(d => defaultRecordJson.set(d.id, JSON.stringify(visitorRecordOf(d))));
  }
  const delta = [];
  courses.forEach(c => {
    const rec = visitorRecordOf(c);
    /* 개수만 비교하면 '시드 후기 하나를 지우고 내 후기 하나를 쓴' 코스를 못 잡아 그 기록이 빠진다.
       통째로 대조해 한 글자라도 다르면 방문자 기록으로 본다. */
    if (JSON.stringify(rec) !== defaultRecordJson.get(c.id)) delta.push(rec);
  });
  /* 델타 길이가 코스 수와 같아지는 단 하나의 경우, 부팅부가 이 배열을 '완본'으로 오인해
     본문 없는 코스를 그대로 쓴다. 그때만 전량 저장으로 되돌린다(방문자 기록 손실 0). */
  const payload = delta.length >= defaultCourses.length ? courses : delta;
  return lsSet("gongacourse_data", JSON.stringify(payload));
}
...
function openPhotoZoom(el) {
  ...
  ov.innerHTML = `<span class="pz-close" aria-label="닫기"><i class="fa-solid fa-xmark" aria-hidden="true"></i></span><img src="${escHtml(src)}" alt="확대 사진">`;
  ov.classList.add("on");
...
/* 홈 계절/날씨 컨텍스트 문자열.
   weatherOnly=true는 계절을 뺀다 — 홈은 추천 카드에 계절 배지가 이미 붙어 있어, 헤더 칩까지 계절을
   적으면 카드 한 장에 같은 계절이 두 번 뜬다. 테마 허브 추천에는 그 배지가 없으므로 계절까지 싣는다. */
function seasonWeatherContext(weatherOnly) {
  const s = getCurrentSeason();
  const parts = weatherOnly ? [] : [`${SEASON_EMOJI[s]} ${SEASON_KO[s]}`];
  if (weatherInfo) parts.push(`${weatherInfo.temp}° ${weatherInfo.emoji}`);
  return parts.join(" · ");
}
...
const SITUATIONS = [
  ...
  { key: "coll:wfall", icon: "fa-water", label: "국내 폭포", cls: "sit-blue" },
...
const HOME_SHORTCUT_FIRST_COUNT = 4;
const HOME_SHORTCUT_DEFAULT_ORDER = SITUATIONS.map(s => s.key);

function normalizedHomeShortcutOrder(order) {
  const valid = new Set(HOME_SHORTCUT_DEFAULT_ORDER);
  const clean = Array.isArray(order) ? order.filter((key, i, arr) => valid.has(key) && arr.indexOf(key) === i) : [];
  HOME_SHORTCUT_DEFAULT_ORDER.forEach(key => { if (!clean.includes(key)) clean.push(key); });
  return clean;
}

function orderedHomeShortcuts() {
  const order = normalizedHomeShortcutOrder(visitorSettings.homeShortcutOrder);
  const byKey = Object.fromEntries(SITUATIONS.map(s => [s.key, s]));
  return order.map(key => byKey[key]).filter(Boolean);
}

/* 「위 4개가 첫 화면에 보여요」는 순서 설정이 방문자에게 한 약속이다. 홈이 12개를 통째로 펼치면
   그 약속도, 별표를 눌러 순서를 바꾼 수고도 화면에 아무 흔적을 남기지 않는다(약속만 있고 구현이 없던 자리).
   첫 화면에는 앞 4개만 내고 나머지는 같은 줄의 칩으로 그 자리에서 펼친다 — 화면 이동도, 도달 못 하는 항목도 없다.
   펼침은 세션 한정이다: 다시 열면 약속대로 4개로 돌아온다. */
let homeShortcutsExpanded = false;
function homeShortcutStripHtml() {
  const all = orderedHomeShortcuts();
  const rest = all.length - HOME_SHORTCUT_FIRST_COUNT;
  const shown = homeShortcutsExpanded ? all : all.slice(0, HOME_SHORTCUT_FIRST_COUNT);
  const chips = shown.map(s =>
    `<button class="sit-chip ${s.cls}" onclick="gotoSituation('${s.key}')">
      <i class="fa-solid ${s.icon}" aria-hidden="true"></i><span>${s.label}</span>
    </button>`).join("");
  if (homeShortcutsExpanded || rest <= 0) return chips;
  return chips + `<button class="sit-chip sit-teal" onclick="expandHomeShortcuts()">
      <i class="fa-solid fa-ellipsis" aria-hidden="true"></i><span>${rest}개 더</span>
    </button>`;
}
// 펼칠 때 홈을 통째로 다시 그리면 추천·후기 사진이 다시 붙어 화면이 깜빡인다 — 이 줄만 갈아 끼운다.
function expandHomeShortcuts() {
  homeShortcutsExpanded = true;
  const host = document.getElementById("home-shortcuts");
  if (host) host.innerHTML = homeShortcutStripHtml();
  else renderHome();
}
...
function homeShortcutSettingsRows() {
  ...
  return order.map((key, index) => {
    ...
    const first = index < HOME_SHORTCUT_FIRST_COUNT;
    return `<div class="hs-row ${first ? "first" : ""}" data-shortcut-key="${key}">
      <div class="hs-name"><i class="fa-solid ${item.icon}"></i><span>${item.label}</span>${first ? '<b>첫 화면</b>' : ""}</div>
      <div class="hs-actions">
        <button type="button" onclick="moveHomeShortcutFirst('${key}')" ${first ? "disabled" : ""} aria-label="첫 화면에 배치: ${item.label}"><i class="fa-solid fa-star"></i></button>
        <button type="button" onclick="moveHomeShortcut('${key}',-1)" ${index === 0 ? "disabled" : ""} aria-label="위로 이동: ${item.label}"><i class="fa-solid fa-arrow-up"></i></button>
        <button type="button" onclick="moveHomeShortcut('${key}',1)" ${index === order.length - 1 ? "disabled" : ""} aria-label="아래로 이동: ${item.label}"><i class="fa-solid fa-arrow-down"></i></button>
      </div>
    </div>`;
  }).join("");
}

function renderHomeShortcutSettings(message) {
  let overlay = document.getElementById("home-shortcut-settings");
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.id = "home-shortcut-settings";
    overlay.className = "hs-overlay";
    overlay.addEventListener("click", e => { if (e.target === overlay) closeHomeShortcutSettings(); });
    document.body.appendChild(overlay);
  }
  overlay.innerHTML = `<div class="hs-dialog" role="dialog" aria-modal="true" aria-labelledby="hs-title">
    <div class="hs-head">
      <div><b id="hs-title">바로가기 순서</b><span>위 4개가 첫 화면에 보여요</span></div>
      <button type="button" class="hs-close" onclick="closeHomeShortcutSettings()" aria-label="설정 닫기"><i class="fa-solid fa-xmark"></i></button>
    </div>
    <div class="hs-list">${homeShortcutSettingsRows()}</div>
    <p class="hs-live" aria-live="polite">${message || ""}</p>
    <div class="hs-foot">
      <button type="button" class="hs-reset" onclick="resetHomeShortcutOrder()"><i class="fa-solid fa-rotate-left"></i> 기본 순서</button>
      <button type="button" class="hs-done" onclick="closeHomeShortcutSettings()">완료</button>
    </div>
  </div>`;
  return overlay;
}

function openHomeShortcutSettings() {
...
function closeHomeShortcutSettings() {
...
function onHomeShortcutSettingsKeydown(e) {
  if (e.key === "Escape") closeHomeShortcutSettings();
}

function refreshHomeShortcutSettings(message) {
  renderHome();
  const overlay = renderHomeShortcutSettings(message);
  overlay.classList.add("on");
}

function moveHomeShortcut(key, delta) {
  const order = normalizedHomeShortcutOrder(visitorSettings.homeShortcutOrder);
  const from = order.indexOf(key);
  const to = Math.max(0, Math.min(order.length - 1, from + delta));
  if (from < 0 || from === to) return;
  order.splice(to, 0, order.splice(from, 1)[0]);
  saveHomeShortcutOrder(order);
  refreshHomeShortcutSettings("순서를 변경했어요.");
}

function moveHomeShortcutFirst(key) {
  const order = normalizedHomeShortcutOrder(visitorSettings.homeShortcutOrder).filter(x => x !== key);
  order.unshift(key);
  saveHomeShortcutOrder(order);
  refreshHomeShortcutSettings("첫 화면에 배치했어요.");
}

function resetHomeShortcutOrder() {
  saveHomeShortcutOrder(HOME_SHORTCUT_DEFAULT_ORDER);
  refreshHomeShortcutSettings("기본 순서로 되돌렸어요.");
}
...
/* 동행 맞춤 — 홈에 실제로 뜨는 목록 하나로만 말한다.
   전에는 "N개 우선 정렬 중"이라고 확인해 주면서 정작 홈에는 정렬되는 목록이 없었다(확인만 있고 결과가 없는 자리).
   여기서 고른 목록이 곧 그 문구의 근거다 — 개수와 목록이 같은 계산에서 나오므로 어긋날 수 없다.
   후보는 §4에 따라 현재 계절 풀로 한정한다(한여름에 겨울 코스를 맞춤이라며 내밀지 않는다).
   계절 풀이 비는 것은 코스 데이터가 없을 때뿐 — 그때만 전체에서 고른다. */
const COMPANION_LABEL = { parent: "효도코스", pet: "반려견 동반", child: "유아 동반" };
function companionMatches() {
  const comp = visitorSettings.companion;
  if (!COMPANION_LABEL[comp]) return null;
  const season = getCurrentSeason();
  const seasonPool = courses.filter(c => c.season === season);
  const pool = seasonPool.length ? seasonPool : courses;
  let fit;
  if (comp === "parent") fit = pool.filter(isEasy);
  else if (comp === "pet") fit = pool.filter(isPetFriendly);
  else fit = pool.filter(c => parseHours(c.duration) <= 2.0);
  // 탐색 목록과 같은 정렬기를 태워 두 화면의 순서가 갈라지지 않게 한다.
  return { label: COMPANION_LABEL[comp], list: applyPersonalSort(fit) };
}
...
function renderHome() {
  ...
  const cm = companionMatches();

  const sitHtml = homeShortcutStripHtml();

  /* 동행 맞춤 목록 — 개수와 목록이 같은 계산에서 나오므로 "N개"와 화면이 어긋날 수 없다.
     오늘의 추천으로 이미 크게 뜬 코스는 빼서 한 화면에 같은 코스가 두 번 뜨지 않게 한다. */
  let matchHtml = "";
  if (cm) {
    const rows = cm.list.filter(c => !t || c.id !== t.id).slice(0, 3);
    const body = rows.length ? rows.map(c => `
      <div class="feed-item" role="button" tabindex="0" onclick="openCourse(${c.id})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openCourse(${c.id});}">
        <div class="fi-av"><i class="fa-solid fa-person-walking"></i></div>
        <div class="fi-body">
          <div class="fi-text">${c.title}</div>
          <div class="fi-meta">${c.location} · ${c.duration} · ${c.difficulty}</div>
        </div>
      </div>`).join("")
      : `<div class="mc-result"><i class="fa-solid fa-circle-info"></i> 지금 계절에는 조건에 맞는 코스를 찾지 못했어요.</div>`;
    /* '더보기' 링크는 붙이지 않는다 — 탐색에는 동행 조건으로 좁히는 필터가 없어서,
       그 링크를 누르면 개수도 목록도 다른 화면이 나온다(고치려던 "확인만 있고 결과가 없는" 자리를 다시 만드는 셈). */
    matchHtml = `
    ${sectionLabel(`${cm.label} <b>${cm.list.length}개</b>`)}
    <div id="home-match">${body}</div>`;
  }
  ...
  /* 예전에는 여기에 "N개 우선 정렬 중" 확인줄을 한 번 더 뒀다. 같은 개수를 두 번 적는 데다
     그 문구가 가리키는 목록이 홈에 없었다 — 확인줄 대신 아래 맞춤 목록 하나로 말한다. */
  ...
  /* 헤더 칩은 오늘 날씨만 싣는다 — 계절은 바로 아래 카드의 배지가 이미 말한다(카드 한 장에 계절 두 번 금지).
     날씨를 못 받았으면(위치 거부·오프라인) 빈 칩을 두지 않는다. */
  const ctx = seasonWeatherContext(true);
  const ctxChip = ctx ? `<span class="hl-ctx">${ctx}</span>` : "";

  root.innerHTML = `
    ${sectionLabel("☀️ 오늘의 추천", ctxChip)}
...
function drawerTab(group, label, valueText) {
  ...
  return `<button type="button" class="ftab ${open ? "open" : ""} ${has ? "has" : ""}" aria-expanded="${open}" onclick="toggleDrawer('${group}')">
    ...
    <div class="d-card">
      ...
      <div class="review-form">
        <div class="rate-rows">
          ...
          ${RATING_METRICS.map(m => rateRow(m.k, `<i class="fa-solid ${m.ic}" aria-hidden="true"></i> ${m.n}`)).join("")}
        </div>
        <div class="upload-preview" id="upload-preview" style="display:${uploadedPhotoBase64 ? "block" : "none"};">
          ...
          <img id="upload-img" src="${escHtml(uploadedPhotoBase64 || "")}" alt="">
        </div>
...
/* 별점 입력 — 예전에는 22×20px짜리 <i onclick>이라 탭으로 닿지도, 엔터로 누르지도 못했다.
   키보드만 쓰는 방문자에게는 후기에 별점을 매길 방법이 아예 없었다는 뜻이다.
   ARIA 라디오그룹으로 바꾼다: 그룹은 탭 정지 하나만 차지하고(선택된 별만 tabindex=0),
   좌우·상하 화살표로 값을 옮기고, 홈/엔드로 양끝에 간다. 터치 타깃은 44px 이상. */
const STAR_HIT = "min-width:44px;min-height:44px;display:inline-flex;align-items:center;justify-content:center";

function rateRow(metric, label) {
  let stars = "";
  for (let i = 1; i <= 5; i++) {
    stars += `<span class="star" role="radio" aria-checked="false" aria-label="${i}점"`
      + ` tabindex="${i === 1 ? 0 : -1}" data-value="${i}" style="${STAR_HIT}"`
      + ` onclick="setRating('${metric}',${i})" onkeydown="onStarKey(event,'${metric}',${i})">`
      + `<i class="fa-solid fa-star" aria-hidden="true"></i></span>`;
  }
  return `<div class="rate-row"><span class="rate-label" id="rl-${metric}">${label}</span>`
    + `<div class="stars" role="radiogroup" aria-labelledby="rl-${metric}" data-metric="${metric}">${stars}</div></div>`;
}

function setRating(metric, value, focus) {
  activeRatings[metric] = value;
  const g = document.querySelector(`.stars[data-metric="${metric}"]`);
  if (!g) return;
  /* 별 다섯 개를 라디오로 두면 채워진 별이 전부 aria-checked=true가 되어 "5개 선택됨"으로 읽힌다.
     선택된 것은 고른 점수 하나뿐 — 색칠(.on)과 선택 상태를 따로 관리한다. */
  g.querySelectorAll(".star").forEach((s, i) => {
    s.classList.toggle("on", i < value);
    s.setAttribute("aria-checked", i + 1 === value ? "true" : "false");
    // 탭 한 번에 그룹을 지나치게 한다(별 5개가 각각 탭 정지가 되면 폼이 탭 미로가 된다).
    s.tabIndex = (value ? i + 1 === value : i === 0) ? 0 : -1;
  });
  if (focus) { const t = g.querySelector('.star[tabindex="0"]'); if (t) t.focus(); }
}

// 화살표=값 이동, 홈/엔드=양끝, 스페이스·엔터=현재 별 선택
function onStarKey(e, metric, value) {
  const cur = activeRatings[metric] || value;
  let next = null;
  if (e.key === "ArrowRight" || e.key === "ArrowUp") next = cur >= 5 ? 1 : cur + 1;
  else if (e.key === "ArrowLeft" || e.key === "ArrowDown") next = cur <= 1 ? 5 : cur - 1;
  else if (e.key === "Home") next = 1;
  else if (e.key === "End") next = 5;
  else if (e.key === " " || e.key === "Enter") next = value;
  else return;
  e.preventDefault();
  setRating(metric, next, true);
}
...
function renderComments() {
  ...
  cont.innerHTML = currentCourse.comments.map((cm, idx) => {
    let rb = "";
    if (cm.ratings) rb = `<div class="cm-rates">${ratingIcons(cm.ratings)}</div>`;
    // 첨부 사진을 후기 옆에 함께 보여야 어느 글의 사진인지 이어진다(사진첩에만 들어가면 연결이 끊긴다).
    const ph = cm.photo
      ? `<img class="gphoto" src="${escHtml(cm.photo)}" alt="후기에 첨부한 사진" loading="lazy" style="margin-top:6px" onclick="openPhotoZoom(this)" onerror="this.remove()">` : "";
...
/* 경치·편의·주차 지표 — 이모지(⛰️🥾🚗)는 §4 금지이자 스크린리더 소음이다("산 4 등산화 5").
   그림은 Font Awesome + aria-hidden으로 감추고, 뜻과 점수는 항목의 aria-label이 말한다. */
const RATING_METRICS = [
  { k: "scenery", ic: "fa-mountain-sun", n: "경치" },
  { k: "path", ic: "fa-shoe-prints", n: "편의" },
  { k: "parking", ic: "fa-car", n: "주차" }
];
function ratingIcons(rt) {
  return RATING_METRICS
    .map(m => `<span role="img" aria-label="${m.n} ${rt[m.k]}점"><i class="fa-solid ${m.ic}" aria-hidden="true"></i>${rt[m.k]}</span>`)
    .join(" ");
}
...
/* 평점 별 — 이모지·특수문자(★☆)는 스크린리더가 "검은 별 검은 별…"로 읽어 소음이 된다.
   그림은 Font Awesome + aria-hidden으로 감추고, 점수는 호출부의 aria-label이 말한다. */
function commStars(r) {
  if (!r) return "";
  const full = Math.round(r);
  let out = "";
  for (let i = 1; i <= 5; i++) {
    out += `<i class="fa-solid fa-star" aria-hidden="true"${i <= full ? "" : ` style="color:#dfe6e1"`}></i>`;
  }
  return out;
}

function feedItemHtml(it) {
  /* 카드가 div+onclick이면 키보드로는 열 수 없다(탭 이동도, 엔터도 안 먹는다).
     레이아웃을 흔들지 않으려고 클래스는 그대로 두고 역할·포커스·키 처리만 얹는다. */
  const nav = it.courseId
    ? ` role="button" tabindex="0" onclick="openCourse(${it.courseId})"`
      + ` onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openCourse(${it.courseId});}"`
    : "";
  /* 필명·본문·코스명·사진 주소는 전부 방문자가 넣은 값이다 — 이스케이프 없이 innerHTML에 넣으면
     `<b>` 같은 태그가 실제 요소로 살아난다(2026-08-13 실측: 피드에서 주입 성공). */
  const photo = it.photo ? `<div class="fc-photo" style="background-image:url('${escHtml(it.photo)}')" onclick="event.stopPropagation();openPhotoZoom(this)"></div>` : "";
  const ratingBadge = it.rating ? `<span class="fc-rate" role="img" aria-label="평점 ${it.rating.toFixed(1)}점">${commStars(it.rating)} ${it.rating.toFixed(1)}</span>` : "";
  const courseTag = it.courseTitle
    ? `<span class="fc-course"><i class="fa-solid fa-location-dot" aria-hidden="true"></i> ${escHtml(it.courseTitle)}</span>` : "";
  ...
  const mineDel = canDelete(it)
    ? `<button class="fc-del" onclick="event.stopPropagation();${it.kind === "post"
        ...
        : `deleteReviewFromFeed(${it.courseId},${it.cmIndex})`}" aria-label="삭제"><i class="fa-solid fa-trash-can" aria-hidden="true"></i></button>` : "";
  return `
    <div class="feed-card"${nav}>
      <div class="fc-head">
        ${avatarHtml(it, "fc-av")}
        <div class="fc-who"><span class="fc-user">${escHtml(it.user)}</span><span class="fc-date">${escHtml(it.date)}</span></div>
        ${ratingBadge}${mineDel}
      </div>
      ${it.text ? `<div class="fc-text">${escHtml(it.text)}</div>` : ""}
      ${photo}
  ...
  root.innerHTML = `
    ...
    <div class="composer">
      ...
      <div class="composer-photo" id="composer-photo" style="display:none;">
        ...
        <span class="composer-photo-x" role="button" tabindex="0" aria-label="고른 사진 빼기" onclick="clearCommPhoto()"
          onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();clearCommPhoto();}">&times;</span>
      </div>
      <p class="muted-note" id="comm-note" role="alert" style="display:none;text-align:left;padding:6px 0 0;margin:0"></p>
      <div class="composer-actions">
        <button class="composer-link" onclick="triggerCommPhoto('camera')"><i class="fa-solid fa-camera" aria-hidden="true"></i> 사진</button>
        <button class="composer-link" onclick="triggerCommPhoto('gallery')"><i class="fa-solid fa-images" aria-hidden="true"></i> 갤러리</button>
        <button class="composer-post" onclick="submitCommunityPost()"><i class="fa-solid fa-paper-plane" aria-hidden="true"></i> 올리기</button>
      </div>
      <input type="file" id="comm-photo-input" accept="image/*" style="display:none" onchange="handleCommPhoto(event)">
    </div>

    <!-- 하단: 방문후기 정렬 탭 -->
    ${sectionLabel(`<i class="fa-solid fa-comments" aria-hidden="true"></i> 방문후기`)}
    <div class="comm-sort-row" id="comm-sort-row">
      ${SORT_TABS.map(t => `<button class="sort-tab ${commSort === t.k ? "on" : ""}" aria-pressed="${commSort === t.k}" onclick="setCommSort('${t.k}')"><i class="fa-solid ${t.i}" aria-hidden="true"></i> ${t.n}</button>`).join("")}
    </div>
...
/* 소통 피드 노출 상한 — 자유글 + 304코스의 후기를 합치면 698장이 한 화면에 쌓인다(실측
...
function renderCommFeed() {
  ...
  if (!el) return;
  const fkey = [commSort, communityPosts.length].join("|");
...
function setCommSort(k) {
  ...
  /* 이 함수는 목록만 다시 그린다(정렬 줄은 재렌더하지 않는다) — 눌린 상태를 색으로만 바꾸면
     스크린리더에는 어느 탭이 켜졌는지 영영 전해지지 않는다. aria-pressed도 같이 옮긴다. */
  tabs.forEach((t, i) => {
...
// 5) 저장
// -----------------------------------------------------------------------------
// 예방건강 — 오늘의 건강 미션(걸음 연동 2 + 자가 점검 2)
function healthMissions(steps, goal) {
  const remain = Math.max(0, goal - steps);
  const minutes = Math.max(1, Math.ceil(remain / 100)); // ~100보/분
  return [
    { id: "goal", ic: "fa-shoe-prints", t: `오늘 ${goal.toLocaleString()}보 채우기`, d: steps >= goal ? `목표 달성! 훌륭해요 <i class="fa-solid fa-trophy" aria-hidden="true"></i>` : `${remain.toLocaleString()}보 남음 · 약 ${minutes}분만 더!`, done: steps >= goal },
    { id: "brisk", ic: "fa-person-walking", t: "30분 빠르게 걷기", d: "심박을 올리는 활기찬 걸음으로", done: steps >= 3000 },
    /* 수분·스트레칭은 앱이 잴 수 있는 지표가 없다. done:false로 굳혀 두면 영영 완료되지 않는 장식이 되고
       눌러도 아무 일이 없다 — 방문자가 직접 체크하는 항목으로 돌린다(하루 단위, 자정에 새 날로 초기화). */
    { id: "water", ic: "fa-glass-water", t: "수분 1.5L 섭취", d: "걷는 틈틈이 한 모금씩 · 눌러서 완료", done: isMissionChecked("water"), self: true },
    { id: "stretch", ic: "fa-spa", t: "종아리·무릎 스트레칭", d: "운동 전후 5분이면 충분 · 눌러서 완료", done: isMissionChecked("stretch"), self: true }
  ];
}
/* 자가 점검 미션 — 걸음과 같은 저장소(stepData)에 날짜별로 둔다. 날짜 키라서 자정을 넘기면
   자동으로 빈 목록이 되고, 지난 날의 체크는 그대로 남는다. */
function missionChecksOf(k) {
  if (!stepData.checks || typeof stepData.checks !== "object") stepData.checks = {};
  const key = k || dayKey();
  if (!Array.isArray(stepData.checks[key])) stepData.checks[key] = [];
  return stepData.checks[key];
}
function isMissionChecked(id) { return missionChecksOf().indexOf(id) >= 0; }
function toggleMission(id) {
  const arr = missionChecksOf();
  const i = arr.indexOf(id);
  if (i >= 0) arr.splice(i, 1); else arr.push(id);
  saveSteps();
  /* 화면을 통째로 다시 그리면 읽던 자리가 사라진다 — 누른 행과 머리글 카운트만 갱신한다. */
  const el = document.querySelector('.mission[data-mid="' + id + '"]');
  if (!el) { if (currentScreen === "saved") renderSaved(); return; }
  const on = isMissionChecked(id);
  el.classList.toggle("done", on);
  el.setAttribute("aria-pressed", on ? "true" : "false");
  const chk = el.querySelector(".ms-chk i");
  if (chk) chk.className = "fa-solid " + (on ? "fa-circle-check" : "fa-circle");
  const cnt = document.getElementById("mission-count");
  if (cnt) {
    const ms = healthMissions(todaySteps(), stepData.goal);
    cnt.textContent = `${ms.filter(m => m.done).length}/${ms.length} 완료`;
  }
}
// 예방건강 — 계절·날씨 맥락 팁
function healthTip() {
...
function renderSaved() {
  ...
  const missionHtml = missions.map(m => {
    const inner = `<span class="ms-ic"><i class="fa-solid ${m.ic}" aria-hidden="true"></i></span>
      <span class="ms-body"><b>${m.t}</b><span>${m.d}</span></span>
      <span class="ms-chk"><i class="fa-solid ${m.done ? "fa-circle-check" : "fa-circle"}" aria-hidden="true"></i></span>`;
    // 눌리는 것은 버튼이어야 한다 — 자가 점검 항목만 조작 대상이고, 걸음 연동 항목은 표시 전용이다.
    return m.self
      ? `<button type="button" class="mission ${m.done ? "done" : ""}" style="width:100%;text-align:left;line-height:inherit"
          data-mid="${m.id}" aria-pressed="${m.done ? "true" : "false"}" onclick="toggleMission('${m.id}')">${inner}</button>`
      : `<div class="mission ${m.done ? "done" : ""}">${inner}</div>`;
  }).join("");
  const tip = healthTip();
  ...
  // role="tablist" 안의 자식은 role="tab" + aria-selected 가 있어야 스크린리더가 "몇 번째 탭, 선택됨"을 읽는다.
  const seg = ["day", "week", "month"].map(k =>
    `<button type="button" role="tab" aria-selected="${R === k ? "true" : "false"}" class="hh-seg-btn ${R === k ? "on" : ""}" onclick="setHealthRange('${k}')">${k === "day" ? "일" : k === "week" ? "주" : "월"}</button>`).join("");
  ...
  root.innerHTML = `
    <div class="health-hero">
      ...
      ${hhBody}
      <button type="button" class="hh-cta" onclick="openPedometerAndStart()"><i class="fa-solid fa-shoe-prints" aria-hidden="true"></i> 만보기 열어 측정 시작 <i class="fa-solid fa-chevron-right" aria-hidden="true"></i></button>
    </div>

    ${sectionLabel(`<i class="fa-solid fa-person-hiking" aria-hidden="true"></i> 트레킹 활동`, `<span class="sec-sub">저장 코스 완주 시 추정</span>`)}
    ...
    ${sectionLabel(`<i class="fa-solid fa-shield-heart" aria-hidden="true"></i> 예방건강 미션`, `<span class="sec-sub" id="mission-count">${doneCount}/${missions.length} 완료</span>`)}
    ...
    ${sectionLabel(`<i class="fa-solid fa-bookmark" aria-hidden="true"></i> 저장한 코스`, list.length ? `<a onclick="goTab('explore')">더 찾기</a>` : "")}
    ${list.length
...
// 6) 내 정보 — 맞춤설정 + 접근성
// -----------------------------------------------------------------------------
/* 만보기 본문(일/주/월)만 만든다. ⋮ 시트에서 값을 하나 바꿀 때마다 화면 전체를 다시 그리면
   시트가 닫히고 읽던 자리가 맨 위로 튄다 — 본문만 갈아끼우려고 분리해 둔 조각이다. */
function pedBodyHtml() {
  const goal = stepData.goal;
...
// 시트를 닫지 않고 만보기 본문만 갈아끼운다(⋮ 시트의 목표·수동 기록 변경용).
function renderPedBody() {
  const card = document.querySelector(".ped2");
  const head = card && card.querySelector(".ped2-head");
  if (!head) { renderMypage(); return; }
  while (head.nextSibling) card.removeChild(head.nextSibling);
  head.insertAdjacentHTML("afterend", pedBodyHtml());
  card.classList.toggle("measuring", motionActive);
  card.classList.toggle("fill", pedView === "day");
  if (motionActive && !pedDate && !pedSession.timer) {
...
function renderMypage() {
  ...
  if (!root) return;
  const goal = stepData.goal;
  const isToday = !pedDate || pedDate === dayKey();

  // 일/주/월 세그먼트 — tablist 안의 자식은 role="tab"+aria-selected 가 있어야 선택 상태가 읽힌다.
  const seg = ["day", "week", "month"].map(k =>
    `<button type="button" role="tab" aria-selected="${pedView === k ? "true" : "false"}" class="p2-seg-btn ${pedView === k ? "on" : ""}" onclick="setPedView('${k}')">${k === "day" ? "일" : k === "week" ? "주" : "월"}</button>`).join("");

  root.innerHTML = `
    <!-- 만보기 카드 (초록 컨셉 · 일/주/월 + ⋮) -->
    <div class="ped2 ${motionActive ? "measuring" : ""} ${pedView === "day" ? "fill" : ""}">
      <div class="ped2-head">
        <div class="p2-seg" role="tablist" aria-label="기간 선택">${seg}</div>
        <button type="button" class="p2-menu" onclick="togglePedMenu(event)" aria-label="메뉴(수정·공유·설정)"><i class="fa-solid fa-ellipsis-vertical" aria-hidden="true"></i></button>
      </div>
      ${pedBodyHtml()}
    </div>
    ...
    <div class="p2-sheet" id="p2-sheet" hidden>
      <div class="p2-sheet-sec">
        <div class="p2-sheet-t"><i class="fa-solid fa-pen-to-square" aria-hidden="true"></i> 수정 · 수동 기록</div>
        <div class="ped-quick">
          <button type="button" class="q-chip" onclick="addSteps(1000)">+1,000</button>
          <button type="button" class="q-chip" onclick="addSteps(3000)">+3,000</button>
          <button type="button" class="q-chip" onclick="addSteps(5000)">+5,000</button>
          <button type="button" class="q-chip reset" onclick="setTodaySteps(0)">초기화</button>
        </div>
      </div>
      <div class="p2-sheet-sec">
        <div class="p2-sheet-t"><i class="fa-solid fa-share-nodes" aria-hidden="true"></i> 공유</div>
        <button type="button" class="p2-share" onclick="sharePed()"><i class="fa-solid fa-share-nodes" aria-hidden="true"></i> 오늘 걸음 공유하기</button>
      </div>
      <div class="p2-sheet-sec">
        <div class="p2-sheet-t"><i class="fa-solid fa-gear" aria-hidden="true"></i> 설정</div>
        <div class="ped-sub" id="goal-label">하루 목표</div>
        <div class="goal-row" role="group" aria-labelledby="goal-label">${[6000, 8000, 10000, 12000].map(g =>
          `<button type="button" class="goal-chip ${goal === g ? "on" : ""}" data-goal="${g}" aria-pressed="${goal === g ? "true" : "false"}" onclick="setStepGoal(${g})">${g / 1000}천</button>`).join("")}</div>
        <button type="button" class="set-row toggle-row" id="set-admin" role="switch" aria-checked="${visitorSettings.admin ? "true" : "false"}" onclick="toggleAdmin()" style="width:100%;text-align:left">
          <span><i class="fa-solid fa-user-gear" aria-hidden="true"></i> 관리자 모드 (코스 사진 교체)</span>
          <span class="switch ${visitorSettings.admin ? "on" : ""}"><span class="knob"></span></span>
        </button>
      </div>
      <p class="p2-note"><i class="fa-solid fa-circle-info" aria-hidden="true"></i> 폰을 들고 걸으면 자동 카운트돼요. 측정 중엔 화면을 켜두면 끊김 없이 측정되고, 앱을 다시 열면 이어서 측정해요. 걸음은 매일 자정에 0으로 리셋(지난 기록은 주·월 그래프에 보존).</p>
    </div>

    <!-- 접근성 — ⋮ 시트 세 겹 안쪽이 아니라 화면에 바로 둔다(글자·대비가 필요한 사람이 가장 먼저 찾는 설정이다) -->
    <div class="set-card">
      <h3 class="set-title"><i class="fa-solid fa-universal-access" aria-hidden="true"></i> 접근성</h3>
      <div class="set-row"><span id="font-label">글자 크기</span>
        <div class="seg small" role="group" aria-labelledby="font-label">${fontBtn("medium", "보통")}${fontBtn("large", "크게")}${fontBtn("xlarge", "아주크게")}</div>
      </div>
      <button type="button" class="set-row toggle-row" id="set-contrast" role="switch" aria-checked="${visitorSettings.highContrast ? "true" : "false"}" onclick="toggleContrast()" style="width:100%;text-align:left">
        <span><i class="fa-solid fa-circle-half-stroke" aria-hidden="true"></i> 고대비 모드</span>
...
/* 스위치 행 하나만 갱신 — 설정 하나 바꿀 때마다 화면을 통째로 다시 그리면
   열어 둔 ⋮ 시트가 닫히고 읽던 자리가 맨 위로 튄다(G22-07). */
function syncSwitchRow(id, on) {
...
// 선택형 버튼 묶음(목표·글자 크기)의 선택 표시만 갱신
function syncChoiceButtons(selector, isOn) {
...
// 헬스 탭의 '만보기 열어 측정 시작' — 문구대로 화면 이동 + 실제 측정 시작까지 한다.
// (센서 미지원·권한 거부는 toggleStepTracking이 안내한다 — 조용히 탭만 옮기지 않는다.)
function openPedometerAndStart() {
  pedDate = null;          // 과거 날짜를 보던 중이면 오늘로 되돌린다
  pedView = "day";         // 측정 화면은 '일' 보기에만 있다
  goTab("mypage");
  if (!motionActive) toggleStepTracking();
}

function fontBtn(val, label) {
  const active = visitorSettings.fontSize === val;
  return `<button type="button" class="seg-btn ${active ? "on" : ""}" data-font="${val}" aria-pressed="${active ? "true" : "false"}" onclick="setFontSize('${val}')">${label}</button>`;
}
...
function renderAppbarProfile() {
  ...
  el.innerHTML = `
    ...
    <button class="ap-avatar ${avatar ? "has" : ""}" onclick="openProfileEdit()" aria-label="프로필 등록"
      style="${avatar ? `background-image:url('${escHtml(avatar)}')` : ""}">
      ${avatar ? "" : `<i class="fa-solid fa-user-astronaut" aria-hidden="true"></i>`}
      <span class="ap-cam"><i class="fa-solid fa-camera" aria-hidden="true"></i></span>
    </button>
  `;
}
...
function openProfileEdit() {
  ...
  const avatar = visitorSettings.avatar || "";
  /* 이스케이프는 escHtml 하나로 통일한다 — 여기 있던 지역 esc()는 큰따옴표만 바꿔,
     필명에 넣은 `&`가 두 번 해석되고 다른 자리(피드·앱바)와 규칙이 어긋났다. */
  ov.innerHTML = `
    <div class="pm-card" role="dialog" aria-modal="true" aria-label="프로필 등록">
      ...
      <div class="pm-avwrap">
        <button class="pm-av ${avatar ? "has" : ""}" onclick="triggerAvatar()" aria-label="프로필 사진 선택"
          style="${avatar ? `background-image:url('${escHtml(avatar)}')` : ""}">
          ${avatar ? "" : `<i class="fa-solid fa-user-astronaut" aria-hidden="true"></i>`}<span class="pm-cam"><i class="fa-solid fa-camera" aria-hidden="true"></i></span>
        </button>
      ...
      <label class="pm-field"><span>필명</span>
        <input type="text" id="pm-nick" maxlength="16" placeholder="예: 나들이 대장님" value="${escHtml(visitorSettings.nick)}">
      </label>
      <label class="pm-field"><span>휴대폰 번호 (선택)</span>
        <input type="tel" id="pm-phone" inputmode="numeric" maxlength="13" placeholder="010-0000-0000" value="${escHtml(visitorSettings.phone)}">
      </label>
...
function avatarHtml(item, cls) {
  ...
  if (av && isMine(item)) return `<div class="${cls} has" style="background-image:url('${escHtml(av)}')"></div>`;
  return `<div class="${cls}"><i class="fa-solid fa-user" aria-hidden="true"></i></div>`;
}
...
function setFontSize(size) {
  ...
  lsSet("gongacourse_visitor_settings", JSON.stringify(visitorSettings));
  applyFontSize();
  // 글자 크기는 CSS 변수로 즉시 반영된다 — 다시 그릴 것은 선택 표시뿐이다.
  syncChoiceButtons(".seg-btn[data-font]", b => b.dataset.font === size);
}
...
function toggleContrast() {
  ...
  syncSwitchRow("set-contrast", visitorSettings.highContrast);
}
...
// checks: 자가 점검 미션(수분·스트레칭)의 날짜별 체크 목록 — 걸음과 같은 날짜 키를 쓴다.
let stepData = { goal: 8000, days: {}, hours: {}, checks: {}, tracking: false };
function loadSteps() {
  try {
    const s = lsGet("gongacourse_steps");
    if (s) stepData = Object.assign({ goal: 8000, days: {}, hours: {}, checks: {}, tracking: false }, JSON.parse(s));
  } catch (e) { stepData = { goal: 8000, days: {}, hours: {}, checks: {}, tracking: false }; }
...
/* 아래 셋은 전부 ⋮ 시트 안에서 눌린다. renderMypage()로 화면을 통째로 다시 그리면
   시트가 닫히고 스크롤이 맨 위로 튀어, 값 하나 고칠 때마다 시트를 다시 열어야 했다(G22-07).
   바뀐 것(만보기 본문·선택 표시)만 갱신해 시트를 열어 둔 채 값이 바뀌게 한다. */
function setTodaySteps(v) {
  const k = dayKey();
  const val = Math.max(0, parseInt(v, 10) || 0);
  stepData.days[k] = val;
  if (val === 0) stepData.hours[k] = new Array(24).fill(0); // 초기화 시 시간대도 리셋
  saveSteps(); renderPedBody();
}
...
function setStepGoal(v) {
  ...
  saveSteps();
  syncChoiceButtons(".goal-row .goal-chip", b => +b.dataset.goal === stepData.goal);
  renderPedBody();                   // 진행률·남은 걸음·목표선이 목표값에 걸려 있다
}
function weekDays() {
...
function collectionByKey(key) {
  const col = (key === "wfall")
    ...
    /* 프로토타입 속성("constructor"·"toString" 등)이 모음집 행세를 하고 통과하면
       groups가 없어 렌더가 통째로 터진다 — 실제로 담아 둔 키만 통과시킨다. */
    : (Object.prototype.hasOwnProperty.call(COLLECTIONS, key) ? COLLECTIONS[key] : null);
  return (col && Array.isArray(col.groups)) ? col : null;
}
...
/* 모음집을 찾지 못했을 때의 본문. 그냥 return하면 이전 모음집 내용이 그대로 남아
   '보이는데 눌러도 안 되는' 화면이 되거나(G22-16), 첫 진입이면 백지가 된다(G22-15).
   무엇이 없었는지 말하고 돌아가는 길을 준다. */
function collMissingHtml(what, backLabel, backAction) {
  return `<div class="empty-soft">
    <i class="fa-regular fa-compass" aria-hidden="true"></i>
    <p>${what} 정보를 찾을 수 없어요</p>
    <span>목록이 바뀌었거나 주소가 잘못된 것 같아요.</span>
    <button type="button" class="btn-primary" style="margin-top:12px" onclick="${backAction}">${backLabel}</button>
  </div>`;
}
...
function renderCollection(key) {
  ...
  if (!root) return;
  const col = collectionByKey(key);
  // 먼저 비운다 — 데이터가 없을 때 앞 모음집의 목록이 남아 있으면 눌러도 아무 일이 없다.
  if (!col) {
    root.innerHTML = collMissingHtml("모음집", "홈으로 가기", "goTab('home')");
    return;
  }
  ...
  root.innerHTML = `
    ...
    <input class="coll-search" type="search" placeholder="이 목록에서 찾기" value="${escHtml(collFilter)}" oninput="onCollFilter(this.value)" aria-label="목록 내 검색">
...
const COLL_ACCENT = { wtrail: "summer", golf: "spring", wspot: "autumn", m100: "autumn", isl: "summer", wfall: "summer" };
const COLL_FALLBACK_IK = { wtrail: "hiking,trail,mountain", golf: "golf,course,green", wspot: "landscape,scenic,nature", m100: "mountain,korea", isl: "island,sea,korea", wfall: "waterfall,korea,nature" };
...
function collPhotoError(img) {
  // 실사 URL 실패 → 키워드 실사로 1회 폴백(출처 표기는 함께 제거)
  ...
  if (credit) credit.remove();
  /* 마지막 폴백. 외부 사진(loremflickr)이 죽으면 스트립을 통째로 지워 상세 첫 화면이
     허전해지거나 빈 박스가 남았다 — 코스 카드와 같은 계절 그라데이션으로 자리를 채운다.
     일러스트·이모지를 넣지 않으므로 §4(실사만)를 어기지 않는다. */
  const strip = img.closest(".d-photo-strip");
  const sv = img.dataset.sv || "spring";
  img.remove();                      // 깨진 이미지 아이콘을 남기지 않는다
  if (!strip) return;
  if (strip.querySelector(".dp-sv")) return;
  const fill = document.createElement("div");
  fill.className = "dp-sv sv-" + sv;
  fill.style.cssText = "position:absolute;inset:0";  // CSS 파일 변경 없이 스트립을 채운다
...
function waterfallDetailHtml(it) {
  const w = it.wf;
  if (!w) return "";
  const visitor = w.visitors === "미공개" ? "공개 수치 없음" : `${w.visitors}${w.visitorYear && w.visitorYear !== "-" ? ` · ${w.visitorYear}년` : ""}`;
  return `
    <div class="d-card wf-card">
      <h4 class="d-card-title"><i class="fa-solid fa-route"></i> 접근·운영</h4>
      <div class="wf-grid">
        <div class="wf-fact"><span>권역·코스</span><b>${w.zone}</b></div>
        <div class="wf-fact"><span>유형</span><b>${w.type}</b></div>
        <div class="wf-fact"><span>편도 도보</span><b>${w.walk}분</b></div>
        <div class="wf-fact"><span>추천 계절</span><b>${w.season} · ${w.months}</b></div>
      </div>
      <p class="wf-note"><i class="fa-solid fa-location-dot"></i> ${w.accessMemo}</p>
    </div>

    <div class="d-card wf-card">
      <h4 class="d-card-title"><i class="fa-solid fa-chart-line"></i> 선정 근거</h4>
      <p class="ci-desc">${w.popularity}</p>
      <div class="wf-evidence">
        <span>종합 ${w.score}점</span><span>근거 ${w.confidence}등급</span><span>${visitor}</span>
      </div>
    </div>

    <div class="d-card wf-card">
      <h4 class="d-card-title"><i class="fa-solid fa-link"></i> 자료 출처</h4>
      <a class="wf-source" href="${w.url}" target="_blank" rel="noopener noreferrer">
        <span>${w.source}</span><i class="fa-solid fa-arrow-up-right-from-square" aria-hidden="true"></i>
      </a>
      <p class="wf-checked">확인일 ${w.checked} · 정부 공인 단일 100대 목록이 아닌 공개자료 기반 연구형 선정입니다.</p>
    </div>`;
}
...
function renderCollItem(ref) {
  ...
  const col = collectionByKey(key);
  const group = col && col.groups[gi];
  const it = group && group.items && group.items[ii];
  /* 잘못된 항목으로 들어오면 본문이 한 글자도 없는 백지가 됐다(G22-15).
     목록을 아는 경우엔 그 목록으로, 모르면 홈으로 돌려보낸다. */
  if (!it) {
    root.innerHTML = col
      ? collMissingHtml(col.t + " 항목", "목록으로 돌아가기", `openCollection('${key}')`)
      : collMissingHtml("모음집 항목", "홈으로 가기", "goTab('home')");
    return;
  }
  ...
  root.innerHTML = `
    ...
    <div class="d-photo-strip">
      <img class="dp-img" src="${collItemPhoto(key, gi, ii, it)}" alt="${key === "wfall" ? (it.photoType === "ai" ? `${it.n} AI 생성 재현 이미지, 실제 촬영 사진 아님` : `${it.n} 실제 사진`) : `${it.n} 사진`}" loading="lazy"
        data-fb="${collItemFallbackPhoto(key, gi, ii, it)}" data-sv="${COLL_ACCENT[key] || "spring"}"
...
function toggleAdmin() {
  ...
  lsSet("gongacourse_visitor_settings", JSON.stringify(visitorSettings));
  // 시트를 연 채 스위치만 바꾼다(만보기 본문은 관리자 모드와 무관하다).
  syncSwitchRow("set-admin", visitorSettings.admin);
}
